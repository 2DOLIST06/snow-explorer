...
export function normalizePisteMapRow(value: unknown, index = 0): PisteMapRow {
  ...
  const stationId = nullableText(row.station_id ?? rawMapping.station_id); const displayUrl = nullableText(row.display_url ?? rawCandidate.display_url);
  ...
  return {
    ...
    candidate, candidate_id: candidateId, candidate_status: status, current_map_url: nullableText(row.current_map_url),
...
export const pisteMapReady = (row: PisteMapRow) => row.candidate_id !== null && row.candidate_status === "pending" && !row.warnings.some(item => item.blocking);
