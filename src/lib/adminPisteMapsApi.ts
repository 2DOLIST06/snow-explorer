...
import { normalizePisteMapRow, normalizePisteMapWorkspace } from "@/lib/anmsmPisteMaps";
...
export async function preparePisteMap(row: PisteMapRow) { const result = await post<{ item: unknown }>(`${ROOT}/prepare`, { external_station_id: row.external_station_id, external_map_id: row.external_map_id }); return normalizePisteMapRow(result.item); }
