...
const numeric = ["altitude_min_m", "altitude_max_m", "ski_area_km", "pistes_count", "green_pistes_count", "blue_pistes_count", "red_pistes_count", "black_pistes_count", "lifts_count"] as const;
const optionalText = ["description", "cover_image_url", "piste_map_url", "forecast_open_date", "forecast_close_date", "season", "source", "verified_at"] as const;
const empty: Form = { name: "", slug: "", status: "draft", description: "", cover_image_url: "", piste_map_url: "", altitude_min_m: "", altitude_max_m: "", ski_area_km: "", pistes_count: "", green_pistes_count: "", blue_pistes_count: "", red_pistes_count: "", black_pistes_count: "", lifts_count: "", forecast_open_date: "", forecast_close_date: "", season: "", source: "", verified_at: "" };
...
export default function SkiAreaEditor({ id }: { id: number | null }) {
  ...
  useEffect(() => { if (id === null) return; void getAdminSkiArea(id).then(({ ski_area: area }) => { const next = { ...empty } as Form; Object.keys(next).forEach(key => { const value = area[key as keyof typeof area]; next[key as keyof Form] = value == null ? "" : String(value); }); setForm(next); setStations(area.stations || []); }).catch(err => setError(err.message)).finally(() => setLoading(false)); }, [id]);
