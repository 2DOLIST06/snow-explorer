...
function ResortPicker({ item, value, onChange }: { item: AnmsmStationMapping; value: AnmsmResort | null; onChange: (value: AnmsmResort | null) => void }) {
  ...
  return <div className="anmsm-resort-picker">
    {value && <div className="anmsm-picked"><span><strong>{value.name}</strong>{value.slug ? ` · ${value.slug}` : ""}</span><button type="button" onClick={() => onChange(null)}>Effacer</button></div>}
...
export default function StationMappings({ onValidateAndPrepare }: { onValidateAndPrepare: (result: AnmsmMappingResult) => Promise<void> }) {
  ...
  const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [confirming, setConfirming] = useState(false); const [removing, setRemoving] = useState<AnmsmStationMapping | null>(null); const [result, setResult] = useState<AnmsmMappingResult | null>(null); const [exactCount, setExactCount] = useState(0);

  const mergeChoices = useCallback((items: AnmsmStationMapping[]) => setChoices(previous => { const next = { ...previous }; items.forEach(item => { if (!(item.externalStationId in next)) next[item.externalStationId] = mappedResort(item.mapping); }); return next; }), []);
  const load = useCallback(async () => { setLoading(true); setError(""); try { const response = pageSize === "all" ? await loadEveryMapping(search, filter) : await listAnmsmStationMappings({ search, status: filter, page, per_page: pageSize }); setData({ items: response.items, stats: response.stats, page: pageSize === "all" ? 1 : response.pagination.page, pages: pageSize === "all" ? 1 : Math.max(1, response.pagination.totalPages), total: response.pagination.total }); mergeChoices(response.items); } catch (e) { setError(e instanceof Error ? e.message : "Chargement impossible."); } finally { setLoading(false); } }, [filter, mergeChoices, page, pageSize, search]);
  ...
  const chosenCount = Object.values(choices).filter(Boolean).length; const remaining = Math.max(0, data.total - selected.size);
  const fetchAll = async (respectFilters = true) => { const response = await loadEveryMapping(respectFilters ? search : "", respectFilters ? filter : ""); mergeChoices(response.items); return response.items; };
  const selectExact = async () => { setBusy(true); setError(""); try { const items = await fetchAll(false); const exactChoices: Record<string, AnmsmResort> = {}; items.forEach(item => { const exact = item.suggestions.find(suggestion => suggestion.matchType === "normalized_exact"); if (exact) exactChoices[item.externalStationId] = suggestionAsResort(exact); }); setChoices(previous => ({ ...previous, ...exactChoices })); setSelected(previous => new Set([...previous, ...Object.keys(exactChoices)])); setExactCount(Object.keys(exactChoices).length); } catch (e) { setError(e instanceof Error ? e.message : "Sélection globale impossible."); } finally { setBusy(false); } };
  ...
  return <section id="correspondances" className="anmsm-mappings" aria-labelledby="mapping-title">
    ...
    <div className="anmsm-stats">{[["Lignes choisies", chosenCount], ["Lignes cochées", selected.size], ["Correspondances exactes", exactCount], ["Restant à contrôler", remaining]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
    ...
    <div className="anmsm-selection"><button className="btn btn--secondary" disabled={busy} onClick={() => void selectExact()}>Sélectionner toutes les correspondances exactes</button><button className="btn btn--secondary" onClick={() => setSelected(previous => new Set([...previous, ...data.items.filter(item => choices[item.externalStationId]).map(item => item.externalStationId)]))}>Tout cocher dans les résultats affichés</button><button className="btn btn--secondary" disabled={busy} onClick={() => void (async () => { setBusy(true); try { const items = await fetchAll(); setSelected(previous => new Set([...previous, ...items.filter(item => choices[item.externalStationId] || mappedResort(item.mapping)).map(item => item.externalStationId)])); } finally { setBusy(false); } })()}>Tout cocher dans tous les résultats</button><button className="btn btn--secondary" onClick={() => setSelected(new Set())}>Tout décocher</button><button className="btn btn--primary" disabled={!selectedRows.length || busy} onClick={() => setConfirming(true)}>Valider les correspondances et préparer les logos</button></div>
    {loading ? <div className="anmsm-loading">Chargement…</div> : <div className="anmsm-mapping-table-wrap"><table className="anmsm-mapping-table"><thead><tr><th>Sélection</th><th>Station ANMSM</th><th>Suggestions</th><th>Station Snow Explorer choisie</th><th>État</th></tr></thead><tbody>{data.items.map(item => { const key = item.externalStationId; const choice = choices[key] || null; const current = mappedResort(item.mapping); return <tr key={key}><td><input type="checkbox" checked={selected.has(key)} disabled={!choice} onChange={() => setSelected(previous => { const next = new Set(previous); next.has(key) ? next.delete(key) : next.add(key); return next; })} /></td><td><div className="anmsm-station"><div className="anmsm-mini-logo">{item.logo?.url ? <img src={item.logo.url} alt="" /> : <span>Sans logo</span>}</div><div><strong>{item.externalName}</strong><small>{key}</small></div></div></td><td>{item.suggestions.length ? <ul className="anmsm-suggestions-inline">{item.suggestions.map(s => <li key={`${s.stationId}-${s.matchType}`}><button type="button" onClick={() => setChoices(previous => ({ ...previous, [key]: suggestionAsResort(s) }))}><strong>{s.name}</strong><span className={`anmsm-suggestion anmsm-suggestion--${s.matchType}`}>{s.matchType === "normalized_exact" ? "Correspondance exacte proposée" : "Correspondance à contrôler"}</span><small>Score : {s.score}</small></button></li>)}</ul> : "Pas de suggestion"}</td><td>{current && <p>Association actuelle : <strong>{current.name}</strong></p>}<ResortPicker item={item} value={choice} onChange={value => { setChoices(previous => ({ ...previous, [key]: value })); if (!value) setSelected(previous => { const next = new Set(previous); next.delete(key); return next; }); }} />{!!item.mapping && <button className="btn btn--danger" onClick={() => setRemoving(item)}>Supprimer la correspondance</button>}</td><td><span className={`anmsm-status anmsm-status--${item.mapping ? "approved" : "pending"}`}>{item.mapping ? "Associée" : "À associer"}</span></td></tr>; })}</tbody></table></div>}
    ...
    <AccessibleModal open={confirming} title="Valider et préparer les logos" busy={busy} onClose={() => setConfirming(false)}><div className="anmsm-modal"><p><strong>{selectedRows.length} correspondance(s)</strong> seront enregistrées, puis leurs logos seront préparés comme candidats. Aucun logo ne sera publié.</p><div className="anmsm-modal-actions"><button className="btn btn--secondary" onClick={() => setConfirming(false)}>Annuler</button><button className="btn btn--primary" onClick={() => void submit()}>Confirmer et préparer</button></div></div></AccessibleModal>
