...
export default function AnmsmLogosAdmin() {
  ...
  const filteredRows = useMemo<AnmsmWorkspaceRow[]>(() => Array.isArray(rows) ? rows.filter(Boolean) : [], [rows]);
  const pageCount = Math.max(1, Math.ceil(filteredRows.length / PAGE_SIZE));
  const visibleRows = useMemo(() => paginateAnmsmRows(filteredRows, Math.min(page, pageCount), PAGE_SIZE), [filteredRows, page, pageCount]);
  const ready = useMemo(() => filteredRows.filter(canPublish), [filteredRows]);
  const resumeRows = filteredRows.some(needsPreparation);
  return <main className="anmsm-admin">
    ...
    {filteredRows.length === 0 ? <p className="notice">Aucun logo à afficher.</p> : <nav className="anmsm-selection" aria-label="Pagination"><button className="btn btn--secondary" disabled={page <= 1} onClick={() => setPage(current => Math.max(1, current - 1))}>Page précédente</button><strong>Page {Math.min(page, pageCount)} sur {pageCount}</strong><button className="btn btn--secondary" disabled={page >= pageCount} onClick={() => setPage(current => Math.min(pageCount, current + 1))}>Page suivante</button></nav>}</>}
  </main>;
}
