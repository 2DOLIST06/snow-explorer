...
import type { ApiAnmsmWorkspace, ApiAnmsmWorkspaceItem, AnmsmResort } from "@/types/anmsmLogo";

const delay = (milliseconds: number) => new Promise(resolve => setTimeout(resolve, milliseconds));
const canPublish = (row: ApiAnmsmWorkspaceItem) => !!row.mapping && row.candidate?.status === "ready" && !row.candidate.error_message;
const needsPreparation = (row: ApiAnmsmWorkspaceItem) => !!row.mapping && !!row.anmsm_logo_url && (!row.candidate || row.candidate.checksum !== row.anmsm_logo_checksum);
const errorText = (error: unknown) => error instanceof Error ? error.message : "Une erreur inconnue est survenue.";
const rowStatus = (row: ApiAnmsmWorkspaceItem, preparing?: boolean) => preparing ? "Préparation…" : !row.mapping ? "À associer" : !row.anmsm_logo_url ? "Sans logo ANMSM" : row.candidate?.status === "published" ? "Déjà publié" : row.candidate?.status === "error" || row.candidate?.error_message ? "Erreur" : canPublish(row) ? "Prêt à publier" : "À préparer";
...
function MappingCell({ row, onMapped }: { row: ApiAnmsmWorkspaceItem; onMapped: (row: ApiAnmsmWorkspaceItem, resort: AnmsmResort) => Promise<void> }) {
...
export default function AnmsmLogosAdmin() {
  const [workspace, setWorkspace] = useState<ApiAnmsmWorkspace | null>(null); const [selected, setSelected] = useState<Set<number>>(new Set());
  ...
  const replaceRow = useCallback((item: ApiAnmsmWorkspaceItem) => setWorkspace(current => current ? ({ ...current, items: current.items.map(row => row.external_station_id === item.external_station_id ? item : row) }) : current), []);
  const refresh = useCallback(async () => { try { const data = await getAnmsmWorkspace(); setWorkspace(data); setApiError(""); return data; } catch (error) { setApiError(`API indisponible : ${errorText(error)}`); return null; } }, []);

  const prepareOne = useCallback(async (row: ApiAnmsmWorkspaceItem) => {
  ...
  const runPreparation = useCallback(async (data: ApiAnmsmWorkspace) => {
    if (running.current) return; running.current = true; stopRequested.current = false; setStopped(false);
    const queue = data.items.filter(needsPreparation); let errors = 0; setProgress({ done: 0, total: queue.length, errors: 0 });
  ...
  const map = async (row: ApiAnmsmWorkspaceItem, resort: AnmsmResort) => {
    try {
      ...
      const data = await refresh(); const updated = data?.items.find(item => item.external_station_id === row.external_station_id); if (updated) await prepareOne(updated);
  ...
  const publish = async () => {
    ...
    try {
      ...
      setWorkspace(current => current ? ({ ...current, items: current.items.map(row => row.candidate && succeeded.has(row.candidate.candidate_id) ? { ...row, mapping: row.mapping && { ...row.mapping, current_logo_url: row.candidate.candidate_preview_url }, candidate: { ...row.candidate, status: "published" } } : row) }) : current);
  ...
  const stats = workspace?.stats; const ready = useMemo(() => workspace?.items.filter(canPublish) || [], [workspace]);
  const resumeRows = workspace?.items.some(needsPreparation) ?? false;
