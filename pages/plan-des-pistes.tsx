        ...
        <section className="passes-layout">
          ...
          <section className="passes-content" aria-live="polite">
            ...
            {selected && <div className="passes-result"><header><div><p className="eyebrow">Plan du domaine</p><h2>Plan des pistes de {selected.name}</h2><p>{selected.region?.name || selected.department?.name || "Station de ski"}</p></div><Link href={`/stations/${selected.slug}`} className="btn btn--secondary">Voir la station <ArrowRight size={17} /></Link></header>
              {mapUrl ? <figure className="pistes-map"><a href={mapUrl} target="_blank" rel="noreferrer"><img src={mapUrl} alt={`Plan des pistes de ${selected.name}`} /></a>{selected.pistes.caption && <figcaption>{selected.pistes.caption}</figcaption>}</figure> : officialMapUrl ? <div className="pistes-official-link"><Map size={38} /><strong>Plan officiel de {selected.name}</strong><p>Le plan est proposé sur le site officiel de la station.</p><a className="btn btn--primary" href={officialMapUrl} target="_blank" rel="noreferrer">Ouvrir le plan <ExternalLink size={17} /></a></div> : <div className="notice notice--warning"><strong>Plan indisponible</strong><span>Aucun plan des pistes n’est actuellement renseigné pour {selected.name}.</span></div>}
            </div>}
          </section>
        </section>
      </main>
