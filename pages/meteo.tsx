  ...
  return (
    <>
      ...
      <section className="weather-layout">
        <aside className="station-picker" aria-label="Choisir une station"><div className="section-heading"><div><p className="eyebrow">Recherche</p><h2>Choisir une station</h2></div>{q && <span>{filteredStations.length} résultat(s)</span>}</div><label className="field"><span>Station ou région</span><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Ex. Auron, Chamonix, Alpes…" /></label>{loadingStations && stations.length === 0 && <div className="skeleton-panel"><div /><div /><div /></div>}<div className="station-list" role="listbox">{filteredStations.map((station) => <div className={selected?.slug === station.slug ? "station-choice is-selected" : "station-choice"} key={station.id || station.slug}><button type="button" role="option" aria-selected={selected?.slug === station.slug} onClick={() => setSelected(station)}><strong>{station.name}</strong><span>{station.region?.name || "Station de ski"}</span></button></div>)}{q && !loadingStations && filteredStations.length === 0 && <div className="empty-state"><strong>Aucune station trouvée</strong><span>Essayez un nom plus court ou une région proche.</span></div>}</div></aside>
